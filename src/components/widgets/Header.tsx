...
const Header = () => {
  ...
  const scrollToSection = (id: any) => {
    ...
    if (targetSection) {
      targetSection.scrollIntoView({
        ...
        inline: 'end', // You can use 'start', 'center', 'end', or 'nearest'
      });
      setActiveLink(id);
    }
  };
  ...
  return (
    <header
      className={`top-0 z-40 mx-auto w-full flex-none bg-white transition-all duration-100 ease-in dark:bg-black-900 md:bg-white/90 md:backdrop-blur-sm dark:md:bg-black-900/90 ${
        isSticky ? 'sticky' : 'relative'
      }`}
      id="header"
    >
      <div className="mx-auto  w-full border border-black-800  rounded-3xl max-w-5xl py-1 mt-5 px-3 md:flex md:justify-between md:py-1.5 md:px-4">
        ...
        <nav
          ...
          } w-auto overflow-y-auto dark:text-black-200 md:mx-5 md:flex md:h-auto md:items-center md:overflow-visible`}
          aria-label="Main navigation"
        >
          <ul className="flex w-full flex-col pt-8 text-xl md:w-auto md:flex-row md:self-center md:pt-0 md:text-base">
            {links &&
              links.map(({ label, href, elementId, icon: Icon, links }, index) => (
                  ...
                  {links && links.length ? (
                    <>
                      ...
                      <ul
                        ...
                        } rounded pl-4 font-medium drop-shadow-xl md:absolute md:min-w-[200px] md:bg-white/90 md:pl-0 md:backdrop-blur-md dark:md:bg-black-900/90`}
                      >
                    ...
                    <Link
                      className={` ${
                        activeLink === elementId ? 'active-link' : ''
                      }   flex cursor-pointer items-center px-4 py-3 font-medium transition duration-150 ease-in-out hover:text-secondary-500 dark:hover:text-secondary-500`}
                      href={`#${elementId}`}
